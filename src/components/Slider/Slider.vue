<template>
  <div class="Slider" :class="sliderClasses">
    ...
    <div
      ...
      v-if="!settings.disabled && (settings.navButtons || settings.dots)"
    >
      <button
        type="button"
        ref="prevButton"
      ...
      <ul
        ref="dots"
        v-if="settings.dots && !settings.disabled"
        class="Slider__dots"
      >
        <li
        ...
        >
          <button @click="goTo(n - 1), restartAutoPlay()" type="button">
            {{ n }}
          </button>
        </li>
      </ul>
...
export default {
  ...
  computed: {
    ...
    clonedSlides() {
      return (
        !this.settings.disabled && !this.settings.fade && this.settings.infinite
      );
    },
    breakpoints() {
    ...
    currentBreakpoint() {
      ...
      if (this.initialSettings.mobileFirst)
        return breakpoints.find((item) => item < this.widthWindow) || 0;
